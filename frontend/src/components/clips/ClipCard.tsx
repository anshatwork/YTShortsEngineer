...
import type { ClipResult, EditOperation } from "@/types/api";
import { formatDuration, cn, pathToStaticUrl } from "@/lib/utils";
import { API_HOST_URL } from "@/lib/constants";
import { useEditJobsForClip } from "@/hooks/useEditJob";
...
export function ClipCard({ clip, index, jobId }: ClipCardProps) {
  ...
  // Reflect edit-state: if a completed edit exists for this clip, play &
  // download that output instead of the source clip. The list is ordered
  // newest-first, so the first `done` row is the latest edit.
  const editJobs = useEditJobsForClip(jobId ?? null, clip.clip_id);
  ...
  const sourceUrl = pathToStaticUrl(clip.path);
  const staticUrl = editedUrl ?? sourceUrl;
  const isEdited = !!editedUrl;

  const clipDuration = clip.timestamp_range[1] - clip.timestamp_range[0];
  // Prefer the AI-directed thumbnail generated by the backend (ThumbnailNode);
  // fall back to a client-side captured frame when no thumbnail exists.
  const aiThumbUrl = pathToStaticUrl(clip.thumbnail_path);
  const capturedFrame = useThumbnail(sourceUrl, clipDuration);
  const thumbnail = aiThumbUrl ?? capturedFrame;
  ...
  return (
    <article className="border border-ink bg-paper flex flex-col h-full">
      ...
      <div className="relative aspect-[9/16] bg-paper-2 border-b border-ink overflow-hidden">
        {playing && staticUrl ? (
          ...
          <>
            {/* Thumbnail frame — fills the well when available */}
            {thumbnail && (
              <img
                src={thumbnail}
                alt=""
            ...
            <span className="absolute top-2 left-2 font-mono text-[10px] tracking-[0.18em] text-ink-muted uppercase drop-shadow-sm">
              plate&nbsp;{plateNo}
            </span>

            {/* Duration chip — top-right */}
            <span className="absolute top-2 right-2 px-1.5 py-0.5 bg-paper/90 border border-ink font-mono text-[10px] tracking-[0.08em] text-ink num-tabular drop-shadow-sm">
              {duration}
            </span>
      ...
      <div className="px-3 pt-2.5 pb-2 flex-1 space-y-1.5">
        ...
        {/* Edit-state badge — reflects whether the played video is an edit */}
            ...
            <span className="text-ink">
              ...
              {latestEdit && (
                <span className="text-ink-muted">
                  {" · "}
                  {EDIT_OP_LABELS[latestEdit.operation]}
                </span>
              )}
            </span>
          ) : (
        ...
        {/* Title — fixed 2-line slot so action bars align across cards */}
        <p className="text-[13px] text-ink leading-snug line-clamp-2 min-h-[2.25rem]">
          {clip.title || " "}
        </p>

        {/* Hook — fixed 1-line slot, reserved even when absent */}
        <p className="text-[11px] text-ink-muted leading-snug line-clamp-1 italic min-h-[1rem]">
          {clip.hook_text ? `“${clip.hook_text}”` : " "}
        </p>
      </div>
      ...
      <div className={cn(
        ...
        <a
          href={staticUrl ?? "#"}
          download={isEdited ? `${clip.clip_id}_edited.mp4` : `${clip.clip_id}.mp4`}
